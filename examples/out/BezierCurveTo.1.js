(function ()
{
    ...
    function draw()
    {
        clearCanvas(canvas);
        // 随机生成4个点坐标
        const xs = [Math.random(), Math.random(), Math.random(), Math.random()].map(function (i) { return i * canvas.width; });
        const ys = [Math.random(), Math.random(), Math.random(), Math.random()].map(function (i) { return (1 - i) * canvas.height; });
